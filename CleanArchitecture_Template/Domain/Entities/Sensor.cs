...
public class Sensor : BaseEntity
{
      ...
      /// specified <paramref name="uc"/>. The resulting <see cref="Sensor"/> instance is initialized with
      ...
      /// <param name="uc">An implementation of <see cref="ISensorUniquenessChecker"/> used to ensure the sensor's uniqueness.</param>
          ...
          CancellationToken ct = default )
      {
            ...
            await ValidateSensorUniqueness( 0 , trimmedLocation , trimmedName , uc , ct );
            ...
            CancellationToken ct = default )
      {
            ...
            if(Location == trimmedLocation && Name == trimmedName) return; // Keine Änderung

            ValidateSensorProperties( trimmedLocation , trimmedName );

            await ValidateSensorUniqueness( Id , trimmedLocation , trimmedName , uc , ct );
      ...
      public static async Task ValidateSensorUniqueness(
          int id ,
          string location ,
          string name ,
              ISensorUniquenessChecker uc ,
              CancellationToken ct = default )
      {
            if(!await uc.IsUniqueAsync( id , location , name , ct ))
                  throw new DomainValidationException(
                        "Uniqueness" ,
                        "Ein Sensor mit der gleichen Location und dem gleichen Namen existiert bereits." );
      }

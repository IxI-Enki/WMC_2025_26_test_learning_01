...
public sealed class GetAllMeasurementsQueryHandler( IUnitOfWork uow ) :
    IRequestHandler<GetAllMeasurementsQuery, Result<IReadOnlyCollection<GetMeasurementDto>>>
{
    public async Task<Result<IReadOnlyCollection<GetMeasurementDto>>>
        Handle( GetAllMeasurementsQuery request, CancellationToken cancellationToken )
    {

...
\titleformat{\subsubsection}
  {\normalfont\normalsize\bfseries\color{skyblue}}{\thesubsubsection}{1em}{}

% Listen mit weniger Abstand
\setlist[enumerate]{itemsep=0pt,parsep=2pt,leftmargin=*}
\setlist[itemize]{itemsep=0pt,parsep=2pt,leftmargin=*}

% Code-Formatierung
\lstset{
    basicstyle=\small\ttfamily\color{codeblue},
    breaklines=true,
    frame=single,
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{mediumblue}Clean Architecture - Implementation Guide}
\fancyhead[R]{\small\color{mediumblue}\thepage}
...
\begin{titlepage}
    \centering
    \vspace*{2cm}
    {\Huge\bfseries\color{darkblue}Clean Architecture\\[0.3cm]Implementation Steps\par}
    \vspace{1cm}
    {\Large\color{mediumblue}Komplettanleitung\par}
    \vspace{2cm}
    {\large\color{lightblue}
    190 Implementierungs-Schritte\\[0.3cm]
    7 Phasen\\[0.3cm]
    Domain $\rightarrow$ Application $\rightarrow$ Infrastructure $\rightarrow$ API $\rightarrow$ Tests\par}
    \vfill
    {\large\color{skyblue}
    Template-Basis: Sensor/Measurement\\
    Ziel-Implementierung: Library Management (Author/Book/Loan)\par}
    \vspace{1cm}
    {\color{mediumblue}Stand: 2025-11-19\par}
\end{titlepage}
...
\section{Typische Fehlerquellen \& Lösungen}

\begin{tcolorbox}[colback=lightblue!10,colframe=mediumblue]
\begin{itemize}
    \item \textbf{CS0246 (Type not found):} \texttt{using} Statement fehlt $\rightarrow$ Strg+. "`Add using"'
    \item \textbf{CS0535 (Interface not implemented):} $\rightarrow$ Strg+. "`Implement Interface"'
    \item \textbf{EF Core Migration Fehler:} Falsche ConnectionString oder SQL Server nicht erreichbar
    \item \textbf{Null Reference:} Navigation Properties nicht geladen $\rightarrow$ \texttt{.Include()} verwenden
    \item \textbf{Validation Fehler:} FluentValidation Rules prüfen, DTO-Mapping korrekt?
    \item \textbf{Circular Reference:} DTOs statt Entities in API zurückgeben
    \item \textbf{Foreign Key Constraint:} Entities in korrekter Reihenfolge speichern (1-Seite vor n-Seite)
    \item \textbf{Concurrency Exception:} RowVersion in UpdateAsync prüfen, OptimisticConcurrency in EF
\end{itemize}
\end{tcolorbox}

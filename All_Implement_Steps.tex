...
\vspace{1mm}
\noindent\textcolor{skyblue}{\rule{\linewidth}{0.4pt}}
\vspace{-1mm}
\begin{itemize}[leftmargin=*,itemsep=0pt]
    \item \hint{Factory:} Private Ctor + async Factory
    \item \hint{Trimmen:} \texttt{(param??"").Trim()}
    \item \hint{Nav Props:} \texttt{null!} (Required), \texttt{new List<>()} (Collection)
    \item \hint{1:n-Beziehung:} Immer zuerst 1-Seite erstellen, dann n-Seite
    \item \hint{n:m-Beziehung:} Join-Entity + 2x 1:n (z.B. BookAuthor)
\end{itemize}
\vspace{1mm}
...
\begin{enumerate}[resume]
    ...
    \item FluentValidation: \texttt{.AddValidatorsFromAssembly(...)}
    \item ValidationBehavior: \texttt{.AddTransient(typeof(IPipeline<,>)...)}
    \item Mapster-Configs: \texttt{AuthorMappingConfig.Configure...();}
\end{enumerate}
...
\begin{enumerate}[resume]
    \item \texttt{Infrastructure/DependencyInjection.cs}: DbContext registrieren
    \item Repository: \texttt{.AddScoped<IAuthorRepo, AuthorRepo>();}
    \item UnitOfWork: \texttt{.AddScoped<IUnitOfWork, UnitOfWork>();}
    \item Uniqueness: \texttt{.AddScoped<IAuthorUniqueness...>();}
    \item DataSeeder: \texttt{.AddHostedService<StartupSeeder>();}
    \item Options: \texttt{.Configure<StartupSeederOptions>(...)}\end{enumerate}

\vspace{1mm}
\noindent\textcolor{skyblue}{\rule{\linewidth}{0.4pt}}
\vspace{-1mm}
\begin{itemize}[leftmargin=*,itemsep=0pt]
    \item \hint{EF:} \texttt{.Include()} für Nav Props
    \item \hint{FK:} EF setzt automatisch via Nav Prop
    \item \hint{Seed:} Erst \texttt{Add()}, dann 1x \texttt{Save()}
    \item \hint{n:m:} \texttt{HasMany().WithMany().UsingEntity<Join>()}
\end{itemize}
\vspace{1mm}
...
\begin{enumerate}[resume]
    \item \texttt{Api/Program.cs}: Builder, Services registrieren
    \item \texttt{builder.Services.AddApplication();} aufrufen
    \item \texttt{builder.Services.AddInfrastructure(...);} aufrufen
    \item Swagger mit XML-Kommentaren konfigurieren
    \item Middleware: UseSwagger, UseSwaggerUI, MapControllers
\end{enumerate}

\subsection{appsettings.json}

\begin{enumerate}[resume]
    \item \texttt{Api/appsettings.json}: ConnectionString hinzufügen
    \item Seeder-Config: \texttt{"StartupDataSeeder": \{ "CsvPath": "..." \}}
\end{enumerate}
...
\vspace{-1mm}
\begin{itemize}[leftmargin=*,itemsep=0pt]
    \item \hint{InMemory:} DB für schnelle Tests
    \item \hint{Helper:} \texttt{GetFirstAuthorId()}, \texttt{CreateTestBook()}
    \item \hint{AAA:} Arrange-Act-Assert Pattern
    \item \hint{Assert:} \texttt{.Should().BeOfType<Ok>()}
\end{itemize}
\vspace{1mm}
